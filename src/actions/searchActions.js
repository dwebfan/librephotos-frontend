...
export function searchPhotos(query) {
  return function(dispatch) {
    if (query.trim().length === 0) {
      ...
      Server.get(`assets/searchlist?search=${query}`,{timeout:100000})
        .then((response) => {
          var groupedByDate = _.groupBy(response.data,(el)=>{
...
export function searchThingAlbums(query) {
  return function(dispatch) {
    ...
    Server.get(`albums/thing/list/?search=${query}`)
      .then((response) => {
        dispatch({type:"SEARCH_THING_ALBUMS_FULFILLED", payload: response.data})
      })
      .catch((err) => {
...
export function searchPlaceAlbums(query) {
  return function(dispatch) {
    ...
    Server.get(`albums/place/list/?search=${query}`)
      .then((response) => {
        dispatch({type:"SEARCH_PLACE_ALBUMS_FULFILLED", payload: response.data})
      })
      .catch((err) => {
